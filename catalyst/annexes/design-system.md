...
The text link is the privacy page's only element that is not already in this table: ink carrying a 1px underline at a 2px offset, dimming to `text-muted` on hover. The underline is the affordance because no amber step passes as small text on paper except `ember-800`, and a link that has to be told apart from emphasis by hue alone is not a link at a glance (`text-link`, feature 010).
