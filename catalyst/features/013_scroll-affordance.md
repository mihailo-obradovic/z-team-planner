...
- `web/components/HeroRosterStrip.vue`: the rail and ribbon call sites; `web/components/HeroDetailDialog.vue`: the body call site; `HeroStatsPanel.vue`, `HeroPowersPanel.vue` and `HeroNotesPanel.vue`: one each, the stats and powers panels being the two that restructure.
