...
| usage loading                                | `PORTRAIT_LOADING[usage]` | `web/config/portraits.ts`                | `eager` for `card` only, since the overview is the landing tab with its first pairs in view; `lazy` for every usage in the dialog, a later tab, or the picker                                   |
