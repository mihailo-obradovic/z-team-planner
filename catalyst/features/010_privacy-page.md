...
| controller name, contact | literal strings | the maintainer                      | **Mihailo Obradović**; `mihailo.obradovic@pm.me` as the private channel, the repository's GitHub issues page as a public second one |
...
- The planner and the shared-build page each end with a single muted **Privacy** text line below their content — the last thing reached by scrolling, present signed in and signed out, on the prerendered `/` and after hydration, at every breakpoint. It costs the shell no height.
...
- `test/nuxt/privacy-page.test.ts`: the route renders; the **Privacy** line is present on `/` and `/b/{id}` in both auth states and points at `/privacy`; the page contains the retention window, the deletion path and the contact address.
