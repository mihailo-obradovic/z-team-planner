...
- `web/composables/hero/useHeroDerived.ts`: one hero read out of that state for the two surfaces that show the same hero, so the card and the detail dialog cannot disagree about a level, a bonus or a flight lock.
- `web/components/hero/HeroCard.vue` (per-hero controls), `web/pages/index.vue` (roster layout and tabs).
