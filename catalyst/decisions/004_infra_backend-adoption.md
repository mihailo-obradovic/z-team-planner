...
Builds lived only in the browser: localStorage plus a `?build=` share link. Clearing browser data loses them, and a build cannot follow its author to another device. The wanted difference is small and specific — **builds can be saved on the server after logging in** — but it needs three layers decision 001 declined, and it contradicted `context/product-description.md`, which listed accounts and server-side storage as non-goals. Decision 001 reserved this for a new record rather than an edit.
...
**Deployment stays declined.** With a managed database and a hosted IdP there is no multi-service run to orchestrate; only the API needs a host, and that is its own effort (decision 007).

## Scope

This record; the Technical Stack table and ADR index in `project-summary.md`; four stack documents copied into `stacks/` (`backend/python-fastapi.md`, `database/postgres.md`, `_lang/python/python.md`, `_lang/python/toolchain.md`); pruning `architecture.md`'s Stack Modules table to what the bundle carries; `context/product-description.md`'s non-goals; a new `operations.md`; and the `app/` → `web/` rename that makes room for the FastAPI application at the root. No behavior contract changes — the serialized build format is untouched; the API's contracts arrive as feature documents.
...
The four stack documents are byte-identical to the Catalyst release they came from (`cmp`); every `stacks/` link in `architecture.md` resolves (scripted check); `catalyst/tools/validate.py .` reports 0 errors with `operations.md` present. After the `web/` move: oxfmt, oxlint and vue-tsc exit 0, vitest green, and a production build prerenders `/`. The service itself is decision 005's.
