...
// * UModal teleports and only renders its slots when open; stubbing it renders the body and footer inline, which is where everything under test lives.
const STUBS = {
...
async function openDialog() {
  return await mountSuspended(
    defineComponent({
      setup() {
        // ! Inside the component's own Pinia and Nuxt state — a store or `useState` created in the test body is a different instance and every assertion would be vacuous.
