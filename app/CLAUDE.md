...
- **`If-Match` is checked twice, and only the second one counts.** `update_build` compares the header against a build it just read, which catches an ordinary stale save; the guarded `UPDATE ... WHERE updated_at = :held` is what settles two writers who both read the same version before either wrote. Do not "simplify" the second check away.
- **Import validates each item's name itself, and wraps each item in a savepoint.** The name is a plain string on the way in so one unusable one costs its own row rather than the whole offer, and `begin_nested` means an item that fails after writing leaves nothing behind — every rejection today happens before the write, so only a test that forces the other order proves it.
