...
| 014 | Synergy pairs tab             | Draft    | The analysis view behind the empty "Synergy pairs" tab: one full-width card per derived pair — both portraits with the overview's power toggles, the read-only pair total (per-hero stats clamped at 10 before summing), and a combined single-series radar clipping at the rim — with portrait clicks opening the detail dialog in place. | [014_synergy-pairs-tab](features/014_synergy-pairs-tab.md)     |
