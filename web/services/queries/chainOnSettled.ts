 ...
 *
 * Two rules live here so every mutation gets them right in one place: the internal work is
 * **awaited** before the caller's hook runs — otherwise a page's `onSettled` closes its dialog
 * while the list is still stale — and the caller's hook is chained rather than overwritten.
 */
