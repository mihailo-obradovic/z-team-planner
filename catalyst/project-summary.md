...
| 004 | Accounts                       | Approved | Optional Google sign-in via Firebase: a user record with a declared personal-data policy, a profile endpoint, total self-service deletion across both systems, and a one-time offer to keep local builds.                                                 | [004_accounts](features/004_accounts.md)                       |
| 005 | Account builds and share links | Approved | Server-side builds for signed-in users under `/api/v1/builds` — validated against the game data, unique names per account, at most 20, idempotent create and import, ETag-guarded updates — and a live read-only share link `/b/{id}`.                    | [005_account-builds](features/005_account-builds.md)           |
| 006 | Frontend data layer            | Approved | How the Nuxt app reaches the API: one fetcher with a single 401 token-refresh retry, a central error policy, services and Pinia Colada queries per resource, Zod-parsed responses, one Pinia auth store, and the Nitro hero route retired for a constant. | [006_frontend-data-layer](features/006_frontend-data-layer.md) |
...
| 005 | bootstrap   | Proposed    | Bootstrap the FastAPI service — scaffold, health, error envelope, Alembic      | [005_bootstrap_api](decisions/005_bootstrap_api.md)                                     |
