...
| **Slide**           | Text that swaps on a state change: the leaving text travels half a line one way as its opacity drops, the arriving text travels in from the other side as its opacity rises, both at once and clipped to the block's box. `--duration-baseline`, `ease-out` in, `ease-in` out, opacity and `transform` only. The direction is the block's to choose and to state. Feature 025. |
