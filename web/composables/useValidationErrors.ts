...
// * Feature 005 sends `details` on `422` only, each entry a `path` and a `message`, so anything else yields an empty map and the form shows nothing.
export function useValidationErrors(error: Ref<unknown>) {
  return computed<Record<string, string[]>>(() => {
    ...
    return details.reduce<Record<string, string[]>>((accumulator, detail) => {
      // * `data.name` and `name` are the same field to a form; the server's path is dotted and the form's key is the last segment.
