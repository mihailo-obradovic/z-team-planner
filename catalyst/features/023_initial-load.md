...
`/` is prerendered, so its first paint is the planner holding the **default** roster — nobody's build — until hydration runs `loadInitialBuild` and four `ClientOnly` regions mount: a complete, plausible page rearranging itself.
...
- **`/b/**`.** `ssr: false`, so it paints nothing until the bundle runs — feature 026's splash, which restates this feature's annex entry.
...
- **`AuthMenu`'s reserved slot** while sign-in is `unknown` (feature 004). Withholding the cluster defers that empty moment past the reveal rather than removing it.
...
- **A failure timeout.** If the bundle never runs the wait never ends: ending it would reveal a planner whose controls do nothing.
...
- The booting flag is set by the server, by the prerender and by the first client render alike, from the same expression. It is only ever cleared **after** mount, and it has two values rather than presence and absence: unhead does not remove an `<html>` attribute inherited from SSR when its value resolves to `undefined`.
...
- **Development mode.** Nuxt serves styles through JavaScript in dev, so the withheld regions paint for an instant; production's prerendered page does not.
- **A slow device.** The wait is the app's own time to hydrate and load the build; this feature adds no work to it. Against `pnpm dev`, which ships hundreds of unbundled modules, a long ring is that, not a hang. Accepted: revealing sooner means revealing the wrong roster sooner.
- **A visitor with no JavaScript.** One head block undoes every boot rule at once; undoing some but not others would hand them a half-drawn app.
...
- A failed `loadInitialBuild` ends the wait; the leave-site prompt stays unarmed, because there is no loaded build to lose. The throw is caught and logged rather than left to become an unhandled rejection; nothing is shown to the visitor.
...
- `web/components/shell/LoadingRing.vue`: the ring and the layer that centres it.
