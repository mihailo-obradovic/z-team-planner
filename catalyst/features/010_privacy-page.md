...
| controller, contact   | literal strings | the maintainer                      | "a single maintainer", unnamed; the repository's GitHub issues page is the only contact — no name, no email |
| attribution           | prose           | the maintainer                      | a non-profit fan project; all in-game assets are AdHoc Studio's property, all rights reserved               |
...
- A footer bar, a header link, a profile-menu item, or an entry on the error page (feature 009 keeps its single way back). The page exists for legal reach, not discovery — the page-end line and the notice link are the contract.
- Markdown rendering or a CMS.
- Changing what data the app collects. This feature only describes; feature 004 decides.

## User / System Behavior

- The planner and the shared-build page each end with a single muted **Privacy** text line — the last thing reached by scrolling, present signed in and signed out, on the prerendered `/` and after hydration, at every breakpoint. When the content is shorter than its scroll region the line sits at the region's bottom. It costs the shell no height.
...
- The contact link is real and monitored; the page names no person, by the maintainer's choice. A placeholder never ships.
...
- It is prerendered — reachable when the backend is down or, as in stage 1, does not exist.
...
- Stage 1: sign-in is unavailable, so no account data exists yet. The page describes the account behaviour in the present tense anyway — it must be live and accurate _before_ sign-in is switched on.
