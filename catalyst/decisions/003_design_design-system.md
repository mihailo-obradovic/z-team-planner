...
The UI was the Nuxt UI starter skin with ad-hoc edits: seven `@theme` ramps of which only one was used, a font declaration nothing loaded, a `:root` block commented out entirely, and two stale, unannotated component configs — while twelve other rendered components had no config at all. No convention annex existed, so `_vue/style-audit.md` had no tokens to check against.
...
- **One fixed look, no colour modes.** `ui: { colorMode: false }` and no colour-mode button — a deliberate exception to the template's §1 "every color must flip" rule, recorded in the annex.
...
- **Barlow + Barlow Condensed via `@nuxt/fonts`** (root `fonts:` key; it ships with Nuxt UI), self-hosted.
- **Lucide-only stat icons**, replacing raster icons so glyphs inherit `currentColor`.
