...
> **Vendored from upstream** — `catalyst/stacks/frontend/nuxt/ui/nuxtui/nuxtui.md`, Vendoring The Upstream Skill, records the pin and the re-sync procedure. This file and its `references/` are upstream's guidance; where they disagree with the `catalyst/` documents, the `catalyst/` documents win.
