...
| 005 | Account builds and share links | Draft    | Server-side builds for signed-in users under `/api/v1/builds` — validated against the game data, unique names per account, at most 20, idempotent create and import, ETag-guarded updates — and a live read-only share link `/b/{id}`. | [005_account-builds](features/005_account-builds.md)       |
