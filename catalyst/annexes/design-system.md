...
- **Every portrait renders through `HeroPortrait`**, which declares the width per usage site in `web/config/portraits.ts` (feature 021); a new portrait slot adds a usage there, never a `width` at the call site.
...
Seven motions recur and are settled here once, so a second instance of any of them inherits the values instead of re-deciding them.
...
| **Value count**      | A displayed number travelling to its new value rather than cutting to it: 200ms, ease-out cubic, driven in JavaScript rather than CSS. Figures are rounded from the travelling value, and anything that reads a number to decide state reads the settled one.                                                                         |
| **Section collapse** | A panel that keeps its headings and exchanges the content beneath them: each section's **height** closes to nothing and reopens, content clipped rather than squashed, while a heading below travels once to a position measured before the motion starts. Two legs, not three — the travel runs underneath the close and the reopen. |

- **List move** needs a stable identity per element. Where a list holds derived or interchangeable entries (a placeholder, a generated copy), those are keyed by position: they change where they stand rather than travelling, and only the entries with an identity of their own move.
- **A list may carry both a move and a state fade**, and a centred one has to: adding or removing an entry re-centres every other entry, so a fade alone leaves the survivors to jump. The cost is that a leaving element must be **taken out of flow** as its fade begins, or the survivors wait for the fade to finish and jump anyway. Take it out of flow only where the row's size is already reserved, so removing it from the layout cannot resize anything. Where a list is not centred and its entries only ever append or truncate, the move is not worth that: give it the move alone.
...
- **Section collapse** measures its destination first. A motion that discovers where it is going halfway through is a motion that changes direction, which is what riding a collapse up and a reopen back down looks like. Under reduced motion it lands on the new contents with the heading already in place.
...
| Text link          | inline, body type         | —                | 0      | 1px underline   | none          |
...
| ---- | -------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `xl` | + `BUILD CALCULATOR` | three, labelled  | Story Setup · Save · build ▾ · Share · account ▾, all labelled                                                                                                        |
| `lg` | wordmark only        | three, labelled  | as above                                                                                                                                                              |
| `md` | wordmark only        | three, labelled  | icon-only with tooltips; build ▾ keeps a truncated name; the account control is its glyph alone                                                                       |
| base | wordmark only        | none — see below | bare Story Setup, account and menu glyphs; Save · Builds ▾ · Share move to the mobile action bar — Save icon-only at its own square, the other two splitting the rest |

**The account control is the last of the build controls and drops last.** It is the only control whose absence cannot be worked around from elsewhere — a player who cannot reach sign-in at 320px has no account at all — so it keeps a target at every tier, as a bare glyph once the labels are gone. Signed out it is a **Sign in** button, signed in the display name and its menu; both take the same neutral subtle fill as Story Setup (§14.1 — a `secondary` solid on the chrome is 1.29:1 and is never a control here). While the identity is still unknown the control renders at its real geometry and is only `invisible`, so the header does not reflow when the SDK reports — an empty slot that later fills would move every control left of it. **Story Setup sits after it, at the very end of the row**, so its trigger lands under the drawer's own close button at every tier — the two are the same control seen from either side, and a player closing the drawer finds the button that opened it where their finger already is. The account control's own absence is two different things: unknown identity keeps the geometry and only goes `invisible`, while a build with no backend to sign in to omits the control entirely rather than leaving a gap nothing will ever fill.
...
The first threshold is 77rem and not the 78 that 1280 maps to, because 1280 maps to _either_ 1248 or 1238 depending on that scrollbar: at 78rem the desktop layout's own width lands inside the split tier. A threshold that has to hold at a named viewport needs margin for the scrollbar, not the nominal figure.

The numbers come from the panels, not from a grid. Measured at the widest tier: templates 454, requirements 316, math 388 — 1190 including gaps, which is what the stack is capped at below 77rem so the layout does not jump across the threshold. A team slot carrying its classic control row cannot go under 116 (three 24px controls plus padding and border), which puts four of them at a 552px viewport floor; the templates panel still fits four columns at 488 with its stat column squeezed to 100 and nothing clipped, and flips at 487. The team's threshold therefore does **not** coincide with the templates panel's, and that asymmetry is deliberate: each panel changes where its own constraint bites.

Two of those rules have a lower bound of their own rather than ending with the tier that introduced them, and both were found by measuring rather than by reasoning from the ladder. The math panel spans the whole tab below 77rem whether the tab is one column or three, so its two columns hold down to a 510px panel — well past the 49.5rem where the rest of the layout stacks. Its bound is set at a **768px viewport** (a 726px container here) — a judgement about where the split still reads well, not the width where it breaks, and written in px precisely because it is a viewport decision. The templates panel's stat rows went **compact** instead of shedding their wordmarks: label and icon down a type step, the stepper's gaps and value slot tightened, the column gap halved. The 24 × 24 floor is what decides which parts give — the +/- buttons cannot, so everything around them does — and the result carries full stat names to a 320 container, an iPhone SE included. Dropping a label is the last resort below that, and only in the Conditions view, which is the only one that runs two steppers beside it. Reaching for the label first was the wrong instinct twice over: it is the most informative thing in the row and the least obliged to keep its size. A threshold that introduces a rule is rarely the threshold that ends it.

The two-column row is `minmax(454px, 1fr) 1fr`, not `grid-cols-2`. Equal tracks are right while there is room, and below a ~876 container half of it stops holding the templates panel: its `Fail ≥` column left the card by up to 44px, and stayed inside the tab, so nothing at the page level reported it. **An overflow check that measures against the viewport or the outer container will not see a panel clipping its own content** — probe every panel against its own border box. Nor will a border-box probe see a `1fr` cell squeezed under its own text: a fixed track overflows and is caught, a flexible one silently crushes its content instead, so a row mixing the two needs both checks. `minmax(min-content, 1fr)` is not the fix either: this panel has no honest min-content, it overflows instead of pushing back, so the floor has to be its measured width.

Three fixes worth remembering, all of them things that reasoning got wrong and the browser corrected. **Flex cannot break a row without also stretching the item** — the team needs a row of its own at every width while keeping its natural width above 78rem, and `basis-full` gives it both; grid separates them, `col-span-full` for the break and `justify-self` for the fill. **`flex-1` sets the basis to 0**, so a fit-content flex container sizes to its items' max-content rather than their design width: the team slots lost 8px each until the basis went back to the design width with `min-w-0` to allow the shrink. And **a panel whose frame is exactly the panel's design width is the first thing to overflow** — the radar frame was 288 inside a 260 content box at 320, bleeding over the panel's own padding, and `max-w-full` is the whole fix.
...
Component styling is utility-first in the template; a `<style scoped>` block is for what utilities cannot express. When one is needed, its values are `var(--*)` reads from this document, never raw hex or off-scale px. There is no sanctioned `:deep()`: the one exception used to be a third-party radar chart, and decision 008 replaced it with `_shared/StatRadar.vue`, whose SVG reads the token variables directly.
