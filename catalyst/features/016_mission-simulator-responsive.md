...
**28.5rem (≈498px viewport) — the templates panel.** It shows one condition column set at a time behind a `Requirements` / `Conditions` toggle. The row goes **compact** rather than dropping its wordmarks: the label steps down a type size and its icon with it, the stepper tightens its gaps and narrows its value slot, and the column gap halves. The **+/- buttons do not shrink** — they are on the 24 × 24 touch floor (design-system annex §14.2), so the space comes from everything around them. That keeps the stat names on screen at every width worth designing for, an iPhone SE included.

Only the Conditions view ever runs out of room even so, and only below **20rem** — past every such width, but still short of the 320px the app is held to for reflow, which is the one place the icon has to carry the stat alone. The Requirements view never reaches that: one stepper leaves a label room at every width measured.
...
| 320px viewport | four team slots 59 square in one row; radar frame 250; Requirements wordmarks shown, Conditions on icons | the reflow floor |
...
- Every control keeps the 24 × 24 touch-target floor (design-system annex §14.2) at every tier; no tier removes a capability. Where a row has to give up width, the floor is what fixes which parts give: gaps, value slots and type sizes, never the controls themselves.
