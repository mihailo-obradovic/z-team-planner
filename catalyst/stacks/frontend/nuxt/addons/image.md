...
Every `NuxtImg` of a content image names a **preset** — one per usage site (`avatar-header`, `portrait-card`, `gallery-tile`), declared under `image.presets` with `width` in CSS px, `densities: 'x1 x2'`, and `fit: 'cover'`. A preset is where the layout's size lives; inline `width`/`sizes` props on six components are six places that drift.
